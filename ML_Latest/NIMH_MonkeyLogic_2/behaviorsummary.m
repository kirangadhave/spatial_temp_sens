...
uicontrol('parent',hFig,'style','text','units','pixel','position',[x y w 22],'string',['Experiment: ' MLConfig.ExperimentName],'backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');

if isfield(MLConfig,'MLPath')
    [~,Conditions] = fileparts(MLConfig.MLPath.ConditionsFile);
    y = y - lineinterval;
    uicontrol('parent',hFig,'style','text','units','pixel','position',[x y w 22],'string',['Conditions: ' Conditions],'backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');
end

y = y - lineinterval;
uicontrol('parent',hFig,'style','text','units','pixel','position',[x y w 22],'string',['Subject: ' MLConfig.SubjectName],'backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');
...
y = y - lineinterval;
uicontrol('parent',hFig,'style','text','units','pixel','position',[x y w 22],'string',['Start time: ' datestr(StartTime)],'backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');
y = y - lineinterval;
uicontrol('parent',hFig,'style','text','units','pixel','position',[x y w 22],'string',['End time: ' datestr(EndTime)],'backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');
y = y - lineinterval;
uicontrol('parent',hFig,'style','text','units','pixel','position',[x+80 y w-80 22],'string',sprintf('(Elapsed: %s)',datestr(EndTime-StartTime,'HH:MM:SS')),'backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');

y = y - lineinterval;
nblock = length(blockorder);
uicontrol('parent',hFig,'style','text','units','pixel','position',[x y w 22],'string',sprintf('# of completed blocks: %d',nblock),'backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');

correct = sum(0==errortype);
y = y - lineinterval;
uicontrol('parent',hFig,'style','text','units','pixel','position',[x y w 22],'string',sprintf('Success rate: %.2f%% (= %d / %d)',correct/ntrial*100,correct,ntrial),'backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');
...
uicontrol('parent',hFig,'style','pushbutton', 'position',[x+40 y 220 30], 'string','Open MLplayer for replay', 'fontsize',fontsize, 'callback',@launch_mlplayer);
    ...
    function launch_mlplayer(~,~)
        mlplayer(data,MLConfig,TrialRecord,datafile);
    end
end
