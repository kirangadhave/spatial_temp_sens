...
ontarget = eyejoytrack('acquirefix', fixation_point, [fix_radius fix_radius], wait_for_fix);
if ~ontarget
    toggleobject(fixation_point);
    trialerror(4);  % no fixation
    return
end
ontarget = eyejoytrack('holdfix', fixation_point, [hold_radius hold_radius], initial_fix);
if ~ontarget
    toggleobject(fixation_point);
...
ontarget = eyejoytrack('holdfix', fixation_point, [hold_radius hold_radius], sample_time);
if ~ontarget
...
% delay epoch
ontarget = eyejoytrack('holdfix', fixation_point, [hold_radius hold_radius], delay);
if ~ontarget
    toggleobject(fixation_point);
    trialerror(3);  % broke fixation
    return
...
chosen_target = eyejoytrack('acquirefix', [target distractor], [fix_radius fix_radius;fix_radius fix_radius], max_reaction_time);
if ~chosen_target
...
if 1==chosen_target
    ...
    ontarget = eyejoytrack('holdfix', target, [hold_radius hold_radius], hold_target_time);
else
    toggleobject(target);
    ontarget = eyejoytrack('holdfix', distractor, [hold_radius hold_radius], hold_target_time);
end
